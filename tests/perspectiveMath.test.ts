...
describe('rhythm highway perspective', () => {
  it('places notes at the narrow horizon at progress zero', () => {
    const note = getPerspectivePosition(0, 0);
  ...
  it('places notes at the wide judgement line at progress one', () => {
    const note = getPerspectivePosition(3, 1);
  ...
  it('widens lane positions toward the player', () => {
    const horizonGap = getPerspectivePosition(3, 0).x - getPerspectivePosition(0, 0).x;
    const hitGap = getPerspectivePosition(3, 1).x - getPerspectivePosition(0, 1).x;
    expect(hitGap).toBeGreaterThan(horizonGap);
  });
  it('derives every judgement pad from shared hit-line boundaries', () => {
    const top = getHighwayGeometryAtY(PAD_TOP_Y);
    const bottom = getHighwayGeometryAtY(PAD_BOTTOM_Y);
    for (let lane = 0; lane < 4; lane += 1) {
      const pad = getJudgementPadGeometry(lane as 0 | 1 | 2 | 3);
      ...
      expect(pad.centerX).toBe(getHighwayGeometryAtY(pad.centerY).centres[lane]);
    }
  });
  it('keeps logical pad geometry unchanged across physical mobile sizes', () => {
    const baseline = getJudgementPadGeometry(2);
    [[1280, 720], [844, 390], [812, 375], [667, 375]].forEach(() => expect(getJudgementPadGeometry(2)).toEqual(baseline));
  });
});
